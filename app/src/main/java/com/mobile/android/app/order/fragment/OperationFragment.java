...
public class OperationFragment extends LazyFragment {
    ...
    private void refreshData(ArrayList<OrderInfo.OrderBillListInfoBean> datas, boolean b) {
        int size = operationAdapter.getData().size();
        if (!b) {
            operationAdapter.notifyItemRangeRemoved(0, operationAdapter.getData().size());
        }
        operationAdapter.setData(datas);
        if (b) {
            rvOperation.getAdapter().notifyItemInserted(size);
        } else {
            rvOperation.getAdapter().notifyDataSetChanged();
        }
    }
}
