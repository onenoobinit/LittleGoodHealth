...
import com.youyijia.goodhealth.app.order.adapter.NotSendQadapter;
...
public class OperationFragment extends LazyFragment {
    ...
    private NotSendQadapter notSendQadapter;
    ...
    public void finishCreateView(Bundle state) {
        ...
        rvOperation.setOnTouchListener(new View.OnTouchListener() {
            @Override
        ...
        rvOperation.setLayoutManager(manager);
        notSendQadapter = new NotSendQadapter(complains);
        rvOperation.setAdapter(notSendQadapter);

        notSendQadapter.setOnItemChildClickListener((adapter, view, position) -> {
            switch (view.getId()) {
                case R.id.ll_order_item:
                    String oneId = notSendQadapter.getData().get(position).getOrderId() + "";
                    String orderType = notSendQadapter.getData().get(position).getOrderType().getText();
                ...
                case R.id.ll_more_item:
                    String moreId = notSendQadapter.getData().get(position).getOrderId() + "";
                    String textType = notSendQadapter.getData().get(position).getOrderType().getText();
    ...
    private void getMoreWhole() {
        ...
        params.clear();
        params.put("orderStatus", "NON_SEND");
        params.put("limit", "10");
        pageNum += 1;
                ...
                .subscribe(new BaseObserver() {
                    ...
                    protected void onHandleSuccess(BaseEntity baseEntity) {
                        if ("200".equals(baseEntity.getCode())) {
                            ...
                            notSendQadapter.setNewData(complains);
                        } else {
